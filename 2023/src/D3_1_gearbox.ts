const puzzleInput = `311...672...34...391.....591......828.......................738....................223....803..472..................................714.840.
...
interface Kernel { width: number; height: number; data: string[][], numberCoords: number[]; };
...
const getKernel = (previousLine: string[], currentLine: string[], nextLine: string[], charIdx = 0): Kernel => {
    const xMin = charIdx - 2;
    const xMax = charIdx + 2;
    const data = [
    ...
    return {
        ...
        height: data.length,
        data,
        numberCoords: getNumberCoordsOnKernelCenter(data)
    }
}
...
const isKernelNumberAdjacentToSymbol = (kernel: Kernel) => {
    const first = kernel.numberCoords[0];
    const last = kernel.numberCoords[kernel.numberCoords.length - 1];
    ...
    // Check vertical
    for (const xCoord of kernel.numberCoords) {
...
const getKernelNumber = (kernel: Kernel): number => {
    if (!kernel.numberCoords.length) {
        return 0;
    }
...
for (let lineIdx = 1; lineIdx < lines.length - 1; lineIdx++) {
    ...
    for (let charIdx = 2; charIdx < currentLine.length - 2; charIdx++) {
        ...
        if (kernel.numberCoords.length === 0) {
            output += "_";
            continue;
        }
